...
class GSAccount:
    ...
    def add_courses_in_account(self, is_instructor: bool = False, active_cid: list = []) -> None:
        ...
        for course in courses.find_all("a", class_=COURSE_CLASS):
            ...
            for tag in course.parent.previous_siblings:
                if tag.get("class") == "courseList--term pageSubheading":
                    year = tag.body
                    break
            # Change: Only add Courses that matches active_cid.
            if cid in active_cid: 
                self.add_course(cid=cid, name=name, short_name=short_name, year=year)
                logger.debug(f"Course '{name}' (CID: {cid}) was included in Active Course List and will be monitored.")
            else: 
                logger.debug(f"Course '{name}' (CID: {cid}) was not included in Active Course List.")
